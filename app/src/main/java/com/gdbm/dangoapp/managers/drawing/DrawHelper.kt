...
private fun View.getRect(x: Int, y: Int): android.graphics.Rect {
    ...
    return android.graphics.Rect(x, y, width + x, height + y)
}
...
data class DrawBoxContent(val bgColor: Color, val path: List<PathWrapper>)
...
internal suspend fun View.drawBitmapFromView(context: Context, config: Bitmap.Config): Bitmap =
    suspendCoroutine { continuation ->
        doOnLayout { view ->
            ...
            val window =
                (context as? Activity)?.window ?: error("Unable to get window")
                ...
                PixelCopy.request(
                    ...
                    { copyResult ->
                        if (copyResult == PixelCopy.SUCCESS) continuation.resume(this) else continuation.resumeWithException(
                            RuntimeException("Unable to generate bitmap")
                        )
                    },
